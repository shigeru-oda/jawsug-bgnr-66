...
"buildersflash_api_logs_json_express"
"buildersflash_api_logs_json_gz"
"buildersflash_api_logs_json_gz_express"
"buildersflash_api_logs_parquet"
"buildersflash_api_logs_parquet_express"
)
...
WITH t AS (
  SELECT
    ...
    response_time_ms
  FROM {TABLE_NAME}
  WHERE year='2025' AND month='05' AND day BETWEEN '01' AND '07'
)
SELECT
  date_format(ts, '%Y-%m-%d %H:00:00') AS hour_bucket,
  api_path,
  AVG(response_time_ms) OVER (
    PARTITION BY api_path, date_format(ts, '%Y-%m-%d %H:00:00')
    ORDER BY ts
    ROWS BETWEEN 100 PRECEDING AND CURRENT ROW
  ) AS rolling_avg_rt
FROM t
ORDER BY hour_bucket, api_path
LIMIT 5000;
EOSQL
